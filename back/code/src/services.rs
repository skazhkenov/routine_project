...
use actix_web::{web::{self, Data, Json}, Responder, HttpRequest, HttpResponse, cookie::Cookie};
...
async fn get_user_boards(
    ...
    if let Ok(redis_boards_list) = redis_data {
        if redis_boards_list.len() > 0 {
            if headers.contains_key("new_token") {
                ...
                return HttpResponse::Ok().cookie(Cookie::new("x-auth", token)).json(redis_boards_list);
            } else {
    ...
    match result {
        Ok(stored_boards_list) => {
            ...
            if headers.contains_key("new_token") {
                let token = headers.get("new_token").unwrap().to_str().unwrap();
                HttpResponse::Ok().cookie(Cookie::new("x-auth", token)).json(board_list)
            } else {
        ...
        Ok(_) => {
            ...
            if let Ok(redis_tasks_list) = redis_data {
                if redis_tasks_list.len() > 0 {
                    if headers.contains_key("new_token") {
                        let token = headers.get("new_token").unwrap().to_str().unwrap();
                        return HttpResponse::Ok().cookie(Cookie::new("x-auth", token)).json(redis_tasks_list);
                    } else {
            ...
            match result {
                Ok(stored_task_list) => {
                    ...
                    if headers.contains_key("new_token") {
                        let token = headers.get("new_token").unwrap().to_str().unwrap();
                        HttpResponse::Ok().cookie(Cookie::new("x-auth", token)).json(tasks_list)
                    } else {
